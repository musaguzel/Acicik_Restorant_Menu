...
class GalleryViewModel(application: Application) : BaseViewModel(application) {
    ...
    fun getPostData() {
        ...
        if (city.equals("city")) {
            ...
            launch {
                firebaseFirestore.collection("Menu_Tatli").whereArrayContains("city",city.toString()).orderBy("date", Query.Direction.ASCENDING).addSnapshotListener { snapshot, exception ->
                    if (exception != null) {
